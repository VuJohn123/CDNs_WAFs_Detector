// ============================================================
// Multi-CDN/WAF Detector — popup.js
// (Historical per-version bug-fix notes lived in this header comment for
// a while; see CHANGELOG.md for the full, current history instead — this
// comment drifted out of sync with reality across enough releases that
// keeping it updated here too was just another thing to forget.)
// ============================================================

// ── Theme: apply synchronously from a fast local cache before anything
// else runs, to avoid a flash-of-wrong-theme every time the popup opens.
// chrome.storage.local is the source of truth (see the async reconciliation
// further down, near themeToggle) but it's always async — localStorage is
// the only synchronous storage available in an extension page, so it's
// used purely as a same-session cache of "what did we render last time",
// never as the source of truth itself.
try {
  const cachedTheme = localStorage.getItem('cdnwaf_theme_cache');
  if (cachedTheme === 'light') document.documentElement.setAttribute('data-theme', 'light');
  else if (cachedTheme === 'system' && window.matchMedia('(prefers-color-scheme: light)').matches) {
    document.documentElement.setAttribute('data-theme', 'light');
  }
} catch { /* localStorage unavailable (rare, e.g. some managed/locked-down profiles) — falls back to the default dark stylesheet until the async settings reconciliation below applies the real theme */ }
...
const PROVIDER_UI = {
  ...
  flyio: { name:'Fly.io', color:'#8b5cf6', icon:'🪰', groups:[
    { title:'Network', signals:[
      {key:'flyCname',            label:'CNAME → *.fly.dev'},
    ]},
    { title:'Response Headers', signals:[
      {key:'flyRequestId',        label:'fly-request-id present',           tip:'Documented as part of Fly Proxy\'s default response headers; format not independently validated, so treated as strong-but-not-exclusive'},
      {key:'flyRegion',           label:'fly-region present'},
      {key:'flyForwardedPort',    label:'fly-forwarded-port present'},
      {key:'viaFly',              label:'Via header includes fly.io'},
      {key:'serverFlyHeuristic',  label:'Server header hints at Fly (weak, unconfirmed)'},
    ]},
  ]},

  render: { name:'Render', color:'#46e3b7', icon:'🎨', groups:[
    { title:'Network', signals:[
      {key:'renderCname',            label:'CNAME → *.onrender.com',        tip:'Documented official custom-domain target'},
    ]},
    { title:'Response Headers', signals:[
      {key:'renderHeaderHeuristic',  label:'Server/X-Render-Origin-Server hints at Render (weak, unconfirmed)'},
    ]},
  ]},

  railway: { name:'Railway', color:'#9f5fff', icon:'🚆', groups:[
    { title:'Network', signals:[
      {key:'railwayCname',            label:'CNAME → *.up.railway.app',     tip:'Documented default hostname pattern'},
    ]},
    { title:'Response Headers', signals:[
      {key:'railwayHeaderHeuristic',  label:'Server header hints at Railway (weak, unconfirmed exact value)'},
    ]},
  ]},
...
// ── Version tag: always read from the manifest, never hardcoded ──────
// (Fixed a real staleness bug: this literally said "v9.1" in the markup
// for several version bumps in a row before anyone noticed.)
const installedVersion = chrome.runtime.getManifest().version;
const versionTagEl = document.getElementById('versionTag');
if (versionTagEl) versionTagEl.textContent = `v${installedVersion}`;
...
function applyTheme(theme) {
  ...
  try { localStorage.setItem('cdnwaf_theme_cache', theme); } catch {}
}
...
function renderDetail(result, pid) {
  ...
  // else entirely in between, and nothing here can tell those apart for
  ...
  // Crowd-report feedback loop: show what others have already reported for
  // this provider, tallied by note text, and flag any that match a header
  // this exact scan also flagged as unrecognized. Purely informational —
  // this never feeds into detection/scoring, since the Worker's endpoints
  // have no authentication and the data isn't verified.
  chrome.runtime.sendMessage({ action: 'getCrowdReports', providerId: pid }, res => {
    const panel = document.getElementById('crowdReportsPanel');
    if (!panel) return;
    const reports = res?.reports || [];
    if (!reports.length) {
      panel.innerHTML = '<div class="breakdown-note">No community reports for this provider yet — be the first below.</div>';
      return;
    }
    const tally = new Map(); // note text -> count
    for (const r of reports) for (const note of (r.notes || [])) {
      tally.set(note, (tally.get(note) || 0) + 1);
    }
    const thisSeenHeaders = new Set((result.unknownHeaders || []).map(h => `Unrecognized header: ${h}`));
    const rows = [...tally.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([note, count]) => {
        const matchesThisScan = thisSeenHeaders.has(note);
        return `<div class="breakdown-note" style="${matchesThisScan ? 'color:var(--amber,#f0a020);font-weight:600' : ''}">${escHtml(note)} — reported ${count}× ${matchesThisScan ? '(also seen on this scan!)' : ''}</div>`;
      }).join('');
    panel.innerHTML = `<div class="breakdown-note">${reports.length} report${reports.length > 1 ? 's' : ''} submitted by other users for ${escHtml(ui.name)} (unverified — shown for context, not used in scoring):</div>${rows}`;
  });
...
function renderSettings(settings) {
  resultsEl.innerHTML = `
    ...
    <div class="checks-list">
      ...
      <div class="settings-section-title" style="margin-top:16px;opacity:0.7">— About —</div>
      <div class="breakdown-note">CDN/WAF Detector v${installedVersion} — local scoring; crowd-sourced signature reporting is on by default (see above).<br>
  ...
  document.getElementById('crowdEndpointInput').addEventListener('change', e => {
    // Self-heal the exact footgun found in testing: a bare Worker URL with
    // no path silently POSTs to '/', which only serves the dashboard (GET) —
    // the report is "sent" with no error, but never reaches /report and
    // never gets stored. Auto-append /report unless it's already there.
...
// ── Update-available banner (Chrome build only — Firefox gets real
// auto-updates through AMO, so getUpdateStatus reports updateAvailable:
// false there without ever calling GitHub) ──────────────────────
chrome.runtime.sendMessage({ action: 'getUpdateStatus' }, u => {
  if (!u?.updateAvailable) return;
  const banner = document.createElement('div');
  banner.className = 'ambient-banner';
  banner.style.cursor = 'pointer';
  banner.textContent = `⬆ Update available: v${u.remoteVersion} (you have v${u.installedVersion}) — tap to open the repo`;
  banner.addEventListener('click', () => chrome.tabs.create({ url: u.repoUrl }));
  const statusBar = document.getElementById('status-bar');
  if (statusBar) statusBar.parentNode.insertBefore(banner, statusBar);
});
